...
import 'emr_patients_screen.dart';

/// The four boxes on the doctor dashboard. The enum keeps their labels and
/// icons in one place, and _onActionTapped below is the one place that decides
...
class _DoctorDashboardScreenState extends State<DoctorDashboardScreen> {
  ...
  Future<void> _refreshPendingCount() async {
    final count = await AppointmentService.doctorNotificationCount(
      widget.user.id,
    );
  ...
  Future<void> _onActionTapped(DoctorAction action) async {
    ...
    switch (action) {
      ...
      case DoctorAction.emrDetails:
        // Opens the doctor's own patients rather than a box to type a patient
        // into: a record belongs to somebody they have treated, and the list
        // is the only place that relationship exists.
        await Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) => EmrPatientsScreen(doctor: widget.user),
          ),
        );
    }
  }
          ...
          child: Column(
            ...
            children: [
              DashboardHeader(
                ...
                // Everything the bell could ever be counting - a new
                // booking, a patient moving one, a patient calling one off -
                // is a row of the appointments list, so the bell goes there
                // whether or not it has a dot on it. A bell that only worked
                // while something was unread would do nothing the rest of the
                // time, which reads as broken rather than as "nothing new".
                onBellPressed: () =>
