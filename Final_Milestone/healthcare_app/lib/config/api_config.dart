...
class ApiConfig {
  ...
  ///Builds the address of one uploaded file inside /uploads, e.g. a patient's
  ///profile photo.
  ///`ApiConfig.uploadUrl('patient_6_1787234322.jpg')`
  ///  -> http://10.0.2.2/healthcare_api/uploads/patient_6_1787234322.jpg
  ///
  ///The database stores only the file name, never a full address, so the same
  ///row works on the emulator and on a real phone - which reach the server at
  ///two different hosts. The address is put together here, at the one place
  ///that knows which of them is in use.
  static Uri uploadUrl(String fileName) =>
